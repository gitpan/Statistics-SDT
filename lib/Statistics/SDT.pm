...
$VERSION = 0.01;
@EXPORT_OK = qw(dprime aprime area_dprime likelihood_bias log_likelihood_bias decision_bias griers_bias);
...
sub area_dprime {
    ...
    if ( (!$h && !$f) || ($h == 1 && $f == 1) ) {
        ...
        return ndtri(dprime($h, $f, 2) / sqrt(2));
    }
}
...
The number of response alternatives. Default = 2 (for the classic signal-detection situation of discriminating between signal+noise and noise-only). If the number of alternatives is greater than 2, the measure of sensitivity, when calling L<dprime|dprime>, is based on the Smith (1982) algorithms.

=item correct

A parameter that indicates whether or not to perform a correction on the number of hits and false-alarms as a corrective when the hit-rate or false-alarm-rate equals 0 or 1 (due, e.g., to strong inducements against false-alarms, or easy discrimination between signals and noise). This is relevant to all functions that make use of the I<inverse phi> function (all except L<aprime|aprime> and L<griers_bias|griers_bias>). 
...
If set to zero (the default), no correction is performed to the calculation of the rates. This should only be used when using the nonparametric algorithms (L<aprime|aprime> and L<griers_bias|griers_bias>). An alternative to these corrections is, indeed, to use the nonparametric measures.
...
If both the hit-rate and false-alarm-rate are either 0 or 1, then C<dprime> returns 0.
...
The area under the receiver-operating-characteristic (ROC) curve, simply equalling the proportion of correct responses that would have been yielded had the task been a two-alternative forced-choice task rather than a yes/no task.

If both the hit-rate and false-alarm-rate are either 0 or 1, then C<area_dprime> returns 0.5.
...
=head2 Function-oriented style
 ...
 use Statistics::SDT qw(dprime decision_bias);
